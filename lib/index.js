...
function captainHook(schema) {
  ...
  schema.pre('save', function (next) {
    ...
    if (this.isNew) {
      runPreMethods(schema.preCreateMethods, self, function(){
        next();
      });
    } else {
      runPreMethods(schema.preUpdateMethods, self, function(){
        next();
      });
    }
  });
  ...
  schema.post('save', function () {
    var self = this;
    if (this._wasNew) {
      runPostMethods(schema.postCreateMethods, self);
    } else {
      runPostMethods(schema.postUpdateMethods, self);
    }
  });
  ...
  runPreMethods = function(methods, self, callback){
  ...
  runPostMethods = function(methods, self){
    async.eachSeries(methods,
      function(fn, cb) {
        fn(self, cb);
      }, function(err){
